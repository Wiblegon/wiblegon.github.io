...
function odulTuru(baslik) {
  ...
  if (icerir("mekik") || icerir("situp") || icerir("sit-up")) return "mekik";
...
const EGZERSIZ_TURLERI = ["sinav", "mekik"];
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (yol === "/" || yol === "/api/saglik") return cevap({ tamam: true, surum: "medya-v4" }, 200, request, env);
...
async function krediGetir(env, kullanici) {
  ...
  const k = { video: 0, ses: 0, foto: 0, sinav: 0, mekik: 0, tts: 0 };
...
async function overlaySiradaki(request, env) {
  ...
  if (sira.kaynak === "tts") {
    return cevap({ tur: "tts", ses: sira.ses, metin: sira.metin, izleyici: sira.kullanici }, 200, request, env);
  }
...
async function adminKrediler(request, env) {
  const { results } = await env.DB.prepare(
    ...
    "SUM(CASE WHEN tur='mekik' THEN adet ELSE 0 END) AS mekik, " +
